...
document.addEventListener('DOMContentLoaded', async () => {
    // 1. Interface Inicial (Nome, Frase e Data)
    ...
    // 2. Verificação de Manutenção (Real-time)
    ...
    onSnapshot(doc(db, "config", "mural"), (snap) => {
        if (snap.exists()) {
            ...
            // Atualiza o preview no card da Home
            if(muralPreview) muralPreview.innerText = d.texto.substring(0, 35) + "...";

            // Remove ouvintes antigos para não duplicar e adiciona o novo clique
            if (btnMural) {
                btnMural.replaceWith(btnMural.cloneNode(true)); // Limpa eventos antigos
                const novoBtnMural = document.getElementById('btn-mural-main');
                
                novoBtnMural.addEventListener('click', () => {
                    console.log("Abrindo modal do mural...");
                    if (modalMural && muralMsg) {
                        muralMsg.innerHTML = `
                            <div style="text-align: left;">
                                <p style="white-space: pre-wrap; font-size: 15px; color: #ccc; line-height: 1.6;">${d.texto}</p>
                                <div style="margin-top: 20px; padding-top: 15px; border-top: 1px solid rgba(138,43,226,0.2); display: flex; justify-content: space-between; align-items: center;">
                                    <span style="font-size: 11px; color: #666;">${d.data || ''}</span>
                                    <small style="color:#8a2be2; font-weight:bold; font-size: 12px;">AUTOR: ${d.autor || 'Admin'}</small>
                                </div>
                            </div>
                        `;
