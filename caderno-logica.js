...
var resultadoIA = '';
var todasNotas = [];
var idsExcluidos = JSON.parse(localStorage.getItem('dt_caderno_excluidos') || '[]');
...
function carregarNotas() {
    ...
    .then(function(snap) {
        ...
        snap.forEach(function(d) {
            // Ignora notas que foram excluídas localmente
            if (idsExcluidos.indexOf(d.id) !== -1) return;
            var nota = Object.assign({ id: d.id, firebaseId: d.id }, d.data());
            // Busca no local pelo id do Firebase OU pelo campo id interno
            var localNota = local.find(function(n) { return n.id === d.id || n.firebaseId === d.id; });
            if (localNota) {
...
window.novaNota = function() {
    ...
    if (db && userEmail && userType !== 'local') {
        db.collection("caderno").add({
            titulo: '', corpo: '', fixada: false, rotinaDiaria: false,
            usuario: userEmail, criadoEm: ts, atualizadoEm: ts
        }).then(function(ref) {
            var idx = todasNotas.findIndex(function(n) { return n.id === nova.id; });
            if (idx !== -1) {
                todasNotas[idx].id = ref.id;
                todasNotas[idx].firebaseId = ref.id;
                if (notaAtual && notaAtual.id === nova.id) {
                    notaAtual.id = ref.id;
                    notaAtual.firebaseId = ref.id;
                }
            }
            salvarLocal();
        }).catch(function(e) { console.error(e); });
...
window.voltarLista = function() {
    ...
    notaAtual = null;
    // Mostra local primeiro, depois sincroniza Firebase
    renderizarLista(todasNotas);
    if (db && userEmail && userType !== 'local') {
        carregarNotas();
    }
...
function salvarNota() {
    ...
    salvarLocal();
    var fbId = notaAtual.firebaseId || notaAtual.id;
    if (db && userEmail && userType !== 'local' && !fbId.startsWith('local_')) {
        db.collection("caderno").doc(fbId).update({
...
window.confirmarExcluirNota = function() {
    ...
    var fbIdExcluir = notaAtual.firebaseId || notaAtual.id;
    
    // Remove do array e salva local PRIMEIRO
    todasNotas = todasNotas.filter(function(n) { return n.id !== idParaExcluir; });
    salvarLocal();
    
    // Volta pra lista mostrando local (sem buscar Firebase)
    notaAtual = null;
    document.getElementById('tela-editor').style.display = 'none';
    document.getElementById('tela-lista').style.display = 'block';
    renderizarLista(todasNotas);
    ...
    // Registra como excluído pra não voltar do Firebase
    if (fbIdExcluir && !fbIdExcluir.startsWith('local_')) {
        idsExcluidos.push(fbIdExcluir);
        localStorage.setItem('dt_caderno_excluidos', JSON.stringify(idsExcluidos));
    }

    // Deleta no Firebase em background
    if (db && userEmail && userType !== 'local' && fbIdExcluir && !fbIdExcluir.startsWith('local_')) {
        db.collection("caderno").doc(fbIdExcluir).delete()
        .then(function() {
            // Remove da lista de excluídos após confirmar delete
            idsExcluidos = idsExcluidos.filter(function(i) { return i !== fbIdExcluir; });
            localStorage.setItem('dt_caderno_excluidos', JSON.stringify(idsExcluidos));
        })
        .catch(function(e) { console.error('Erro ao deletar:', e); });
    }
};
