...
window.confirmarExcluirNota = function() {
    ...
    salvarLocal();
    var fbIdExcluir = notaAtual ? (notaAtual.firebaseId || idParaExcluir) : idParaExcluir;
    if (db && userEmail && userType !== 'local' && fbIdExcluir && !fbIdExcluir.startsWith('local_')) {
        db.collection("caderno").doc(fbIdExcluir).delete().catch(function(e) { console.error(e); });
    }
